--create a raw table
CREATE EXTERNAL TABLE raw_data_adventure.productcategory (
...
LOCATION '/external_databases/data_adventure/productcategory'
tblproperties("skip.header.line.count"="1");

--create analytics table
CREATE TABLE data_adventure.productcategory (
...
ROW FORMAT DELIMITED
FIELDS TERMINATED BY ','
LOCATION '/databases/data_adventure/productcategory';

--transform data and insert in analytics table
SET hive.exec.dynamic.partition.mode=nonstrict;
INSERT OVERWRITE TABLE  data_adventure.productcategory
PARTITION (modified_year)
select
pcr.*,
year(pcr.modifieddate) as modified_year
from (
    select
    ...
    from raw_data_adventure.productcategory
    ) as pcr;